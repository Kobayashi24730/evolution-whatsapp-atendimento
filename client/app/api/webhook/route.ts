    ...
    async function obterAtendenteDisponivel() {
        //? pega todos os IDs de user com sessoes ativas.
        const usuariosOn = await prisma.session.findMany({
            where: {
                expires: { gte: new Date() } //? Sessão ainda não expirou.
            },
            select: { userId: true }
        });
        const idsOnline = usuariosOn.map((u) => u.userId);
        if (idsOnline.length === 0) {
            return null;
        }
        //? Busca o usuário online e que tem menos atendimentos vinculados.
        const atendentesComContagem = await prisma.user.findMany({
            where: {
                id: { in: idsOnline },
            },
            select: {
                ...
                name: true,
                _count: {
                    select: { atendimentos: { where: { status: { in: ["ABERTO", "EM_ATENDIMENTO", "TRIAGEM"] } } } }
                }
            },
            orderBy: {
                atendimentos: {
                    _count: "asc" //? lista do menor para o maior.
                }
            },
            take: 1 //? pega o primeiro.
        });

        return atendentesComContagem[0] || null;
    }
    ...
    export async function POST(request: Request) {
        try {
            ...
            if (!atendimentoActive) {
                const atendenteSorteado = await obterAtendenteDisponivel(); //? sortea o atendente.
                ...
                atendimentoActive = await prisma.atendimento.create({
                    data: {
                        ...
                        status: "ABERTO",
                        userId: atendenteSorteado ? atendenteSorteado.id : null, //? Vincula o id do atendente ao atendimento.
                        mensagens: {
                            create: {
                                texto: textoMensagem || "",
                                tipo: tipoMidia,
                    ...
                    include: {
                        user: true //? para trazer os dados do usuário vinculado.
                    }
                });
